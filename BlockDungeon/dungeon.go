...
func (d *Dungeon) StampRoom(room Room) {
	...
	useSoftCorners := room.Width >= 5 && room.Height >= 5 && rand.IntN(2) == 0

	for x := 0; x < room.Width; x++ {
		for y := 0; y < room.Height; y++ {
			globalX := x + room.PivotX
			globalY := y + room.PivotY
			...
			if x == 0 || y == 0 || x == room.Width-1 || y == room.Height-1 {
				if useSoftCorners && isCorner {
					d.PlaceBlock(FLOOR, globalX, globalY)
				} else {
					d.PlaceBlock(WALL, globalX, globalY)
				}
			} else {
				d.PlaceBlock(FLOOR, globalX, globalY)
			}
